	...
	<ul class="pagination">
		<li <c:if test="${begin == current}">class="disabled"</c:if>>
			<mylib:link disabled="${begin == current}"
				limit="${pagined.getLimit()}" label="First Page">
				<span aria-hidden="true">&laquo;</span>
			</mylib:link>
			<mylib:link disabled="${begin == current}"
				page="${current - 1}" limit="${pagined.getLimit()}"
				label="Previous">
		...
		<c:forEach begin="${begin}" end="${end}" var="myPage">
			<li <c:if test="${myPage == current}">class="disabled"</c:if>>
				<mylib:link disabled="${myPage == current}" page="${myPage}" limit="${pagined.getLimit()}">${myPage}</mylib:link>
			</li>
		</c:forEach>
		<li <c:if test="${end == current}">class="disabled"</c:if>>
			<mylib:link disabled="${pagined.isLast()}"
				page="${current + 1}" limit="${pagined.getLimit()}"
				label="Next">
